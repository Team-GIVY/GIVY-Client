...
interface StartChallengeCopyCodeScreenProps {
  onBack?: () => void;
  onCopyCode?: () => void;
  onSearchHelp?: () => void;
  onComplete?: () => void;
...
function StartChallengeCopyCodeScreen({
  onBack,
  onCopyCode,
  onSearchHelp,
  onComplete,
  product,
}: StartChallengeCopyCodeScreenProps) {
  // product가 없으면 로딩 또는 에러 표시
  if (!product) {
    return (
      ...
      >
        ...
        >
          ...
          <span
            style={{
              fontFamily: 'Pretendard',
              fontSize: product.productCode.length > 15 ? '14px' : product.productCode.length > 10 ? '16px' : '20px',
              fontWeight: 'bold',
              lineHeight: 1.6,
              letterSpacing: '-0.2px',
              color: '#252525',
...
export default StartChallengeCopyCodeScreen;
